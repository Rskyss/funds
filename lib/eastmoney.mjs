...
import { markStaleNav } from "./dataQuality.mjs";
...
// 收益段只取 6月/1年/3年年化 三段不重叠区间，避免同一波行情重复计分
export function computeRawScore(fund) {
  ...
  // 三段收益一个都没有（无净值 / 成立不满半年 / 停更）→ 不打分。
  // 之前按 0 参与同主题百分位，没有任何数据的基金会得到一个"看起来正常"的分数（2026-09-01：无净值的基金得 82 分）。
...
// 百分位在同主题内计算：不同资产类别（美股/黄金/债券…）不再同池比高低。
// 附带 peerRank（同主题第几名，1=最好）/ peerCount；主题不足 6 只时标"同类样本少"。
export function applyPercentileScores(funds) {
  if (!funds.length) return funds;
  markStaleNav(funds); // 净值停更（落后全站最新净值 14 天以上）与"数据不足"同样不打分、不占同类样本
  for (const f of funds) {
    if (!Number.isFinite(f.rawScore)) f.rawScore = computeRawScore(f);
  }
  const groups = new Map();
  for (const f of funds) {
    if (!Number.isFinite(f.rawScore) || f.navStaleDays) {
      f.score = null;
      f.label = f.navStaleDays ? "净值停更" : "数据不足";
      f.peerRank = null;
      f.peerCount = null;
      continue;
    }
...
// 排行接口每行是逗号分隔的 21+ 个字段，按位置取值。字段数不够或代码不是 6 位数字 → 视为格式漂移，返回 null。
// 之前没有这道守卫：东财增删一列，所有数字整体错位、“格式合法但完全错误”地写进库。
export const RANK_ROW_MIN_FIELDS = 21;
export function parseFundRow(row) {
  const parts = String(row ?? "").split(",");
  if (parts.length < RANK_ROW_MIN_FIELDS || !/^\d{6}$/.test(parts[0] || "")) return null;
  const code = parts[0];
  const name = parts[1];
  ...
  const fund = {
    ...
    pinyin: parts[2] || "",
    date: parts[3] || "",
    nav: parts[3] ? toNumber(parts[4]) : null,      // 无日期净值当缺失（与读写两侧同一规则）
    accumNav: parts[3] ? toNumber(parts[5]) : null,
...
async function fetchQdiiUniverse() {
  const response = await fetchWithTimeout("https://fund.eastmoney.com/js/fundcode_search.js", {
    headers: {
...
export async function fetchQdiiFunds() {
  ...
  const rankedFunds = raw.datas.map(parseFundRow).filter(Boolean);
  const skipped = raw.datas.length - rankedFunds.length;
  if (raw.datas.length && !rankedFunds.length) {
    throw new Error(`基金排行接口格式疑似漂移：${raw.datas.length} 行全部无法解析，已中止刷新以免写入错位数据`);
  }
  if (skipped > 0) console.warn(`[eastmoney] 排行数据有 ${skipped}/${raw.datas.length} 行字段数不足，已跳过`);
...
/**
 * 概况页「基金管理人」→ 东财公司 id + 名称（购买引导用它查官网对照表）。
 * 有公司链接才给 id；只有文字给名称、id 为 null（不猜）；没有这一项返回 null。
 */
export function parseFundCompany(html) {
  const m = String(html || "").match(/基金管理人<\/th>\s*<td[^>]*>([\s\S]{0,300}?)<\/td>/);
  if (!m) return null;
  const link = m[1].match(/<a[^>]*href="[^"]*\/company\/(\d+)\.html"[^>]*>([^<]+)<\/a>/i);
  if (link) {
    const name = link[2].trim();
    return name ? { id: link[1], name } : null;
  }
  const plain = stripTags(m[1]).trim();
  return plain ? { id: null, name: plain } : null;
}
...
export async function fetchFundProfile(code) {
  ...
  const company = parseFundCompany(html);
  return {
    code,
...
export async function fetchAssetAllocation(code) {
  ...
  if (!response.ok) throw new Error(`资产配置页 HTTP ${response.status}`);
  const html = await response.text();
  return parseAssetAllocationHtml(html);
}

// 资产配置表按表头取列：ETF/指数基金没有「存托凭证」列，之前按固定位置取值会把净资产读成存托凭证、净资产变成空（前端显示 0.00 亿）
export function parseAssetAllocationHtml(html) {
  const tableMatch = String(html || "").match(/<table[^>]*class=["']w782 comm tzxq["'][^>]*>[\s\S]*?<\/table>/);
  if (!tableMatch) return [];
  const table = tableMatch[0];
  const headers = (table.match(/<th[^>]*>([\s\S]*?)<\/th>/g) || [])
    .map((c) => decodeHtmlEntities(c.replace(/<[^>]+>/g, " ")).replace(/\s+/g, ""));
  ...
  const pick = (cells, i) => (i >= 0 && i < cells.length ? cells[i] : null);
  const rows = table.match(/<tr[^>]*>[\s\S]*?<\/tr>/g) || [];
  const result = [];
  for (const row of rows) {
    const cellsRaw = row.match(/<td[^>]*>([\s\S]*?)<\/td>/g);
    if (!cellsRaw) continue;
    const cells = cellsRaw.map((c) => decodeHtmlEntities(c.replace(/<[^>]+>/g, " ")).trim());
    const date = pick(cells, idx.date);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date || "")) continue;
    result.push({
      date,
      stock: parseNum(pick(cells, idx.stock)),
      bond: parseNum(pick(cells, idx.bond)),
      cash: parseNum(pick(cells, idx.cash)),
      depositary: parseNum(pick(cells, idx.depositary)),
      netAssetBillion: parseNum(pick(cells, idx.netAsset)),
    });
...
export async function fetchFundFees(code) {
  ...
  // 抓取失败必须抛错而不是返回空：否则脚本会把“空费率”连同 fees_fetched_at 一起落库，之后永久跳过。
  if (!response.ok) throw new Error(`费率页 HTTP ${response.status}`);
  const html = await response.text();
...
export async function fetchFundRiskMetrics(code) {
  ...
  if (!response.ok) throw new Error(`风险指标页 HTTP ${response.status}`);
  const html = await response.text();
...
export async function fetchFundRating(code) {
  ...
  if (!response.ok) throw new Error(`评级接口 HTTP ${response.status}`);
...
// 东财偶发超时/限流：失败后间隔重试（默认 700ms）。
// 东财被打急了会回 HTTP 514（反爬限流），这种情况退避更久（默认 3s），否则立刻重试只会再吃一个 514。
export function isThrottleError(err) {
  return /\b(514|429|503)\b/.test(String(err?.message || ""));
}

export async function withRetry(fn, { retries = 1, delayMs = 700, throttleDelayMs = 3000 } = {}) {
  let lastErr;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      lastErr = err;
      if (attempt < retries) {
        const wait = (isThrottleError(err) ? throttleDelayMs : delayMs) * (attempt + 1);
...
export async function fetchRatingsConcurrently(codes, { concurrency = 8, onProgress } = {}) {
  ...
  const workers = Array.from({ length: Math.min(concurrency, queue.length) }, async () => {
    while (queue.length) {
      ...
      try {
        const rating = await withRetry(() => fetchFundRating(code));
    ...
    while (queue.length) {
      ...
      try {
        const [profile, risk, status] = await Promise.all([
          withRetry(() => fetchFundProfile(code)).catch(() => null),
          withRetry(() => fetchFundRiskMetrics(code)).catch(() => null),
          withRetry(() => fetchFundStatus(code)).catch(() => null),
        ]);
        if (profile) {
...
export async function fetchNavHistory(code, targetCount = 240) {
  ...
  while (all.length < targetCount && pageIndex <= 20) {
    ...
    // 中途失败要抛错：静默 break 会返回半截历史，调用方分不清“只有 20 天”和“抓失败”
    if (!response.ok) throw new Error(`净值历史接口 HTTP ${response.status}（第 ${pageIndex} 页）`);
    const data = await response.json();
...
export async function fetchFundBasicInfo(code) {
  ...
  if (!response.ok) throw new Error(`基本信息接口 HTTP ${response.status}`);
  const data = await response.json();
  const d = data?.Datas;
  if (!d || !d.SHORTNAME) return null; // 已清盘/无此基金时东财返回空对象 {}
  return {
...
export async function fetchFundReturns(code) {
  ...
  if (!response.ok) throw new Error(`历史净值脚本 HTTP ${response.status}`);
  const text = await response.text();
...
// 兜底通道两个渠道的合并规则（纯函数，便于单测）：
// - 基本信息渠道（fundmobapi FundMNNBasicInformation）不带净值日期，且对没有净值的份额会回占位值 DWJZ=1
//   （2026-09-02：003244/003245 美元现钞/现汇入库成 nav=1 无日期），所以它的净值只在拿得到日期时才作为补充
// - 净值渠道（pingzhongdata）给日期、净值、区间收益，以它为准
export function mergeFallbackInfo(fund, basic, returns) {
  let touched = false;
  if (basic) {
  ...
  if (returns) {
    ...
    for (const k of ["return1m", "return3m", "return6m", "return1y", "returnYtd"]) {
      if (returns[k] !== null) fund[k] = returns[k];
    }
    touched = true;
  }
  if (basic && fund.date) {
    if ((fund.nav === null || fund.nav === undefined) && basic.nav !== null) fund.nav = basic.nav;
    if ((fund.accumNav === null || fund.accumNav === undefined) && basic.accumNav !== null) fund.accumNav = basic.accumNav;
  }
  return touched;
}
...
export async function enrichFallbackFunds(funds, options = {}) {
  ...
  if (!targets.length) return { enriched: 0, failed: 0, total: 0, basicOk: 0, returnsOk: 0 };
  const concurrency = options.concurrency || 8;
  const onProgress = options.onProgress || (() => {});
  let enriched = 0;
  let failed = 0;
  let done = 0;
  let basicOk = 0;   // 基本信息渠道（fundmobapi：成立日期/日涨幅/申购费）成功数
  let returnsOk = 0; // 净值渠道（pingzhongdata：净值/区间收益）成功数

  await runWithConcurrency(targets, concurrency, async (fund) => {
    try {
      const [basic, returns] = await Promise.all([
        withRetry(() => fetchFundBasicInfo(fund.code)).catch(() => null),
        withRetry(() => fetchFundReturns(fund.code)).catch(() => null),
      ]);
      if (basic) basicOk++;
      if (returns) returnsOk++;
  ...
  return { enriched, failed, total: targets.length, basicOk, returnsOk };
}
...
export function buildStructuredAnalysis(fund, funds, detail) {
  ...
  // score 已是主题内相对值，跨主题聚合无意义，这里用绝对分 rawScore
