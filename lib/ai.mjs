...
// 出站超时：非流式 90s（规划/点评生成），流式 180s（整段回答含思考），校验 Key 20s。
const AI_TIMEOUT_MS = Number(process.env.DASHSCOPE_TIMEOUT_MS || 90000);
const AI_STREAM_TIMEOUT_MS = Number(process.env.DASHSCOPE_STREAM_TIMEOUT_MS || 180000);
...
export async function generateFundSummary(fund, { model, apiKey: apiKeyArg, signal } = {}) {
  const apiKey = apiKeyArg || process.env.DASHSCOPE_API_KEY;
...
export async function generateFundDetailSummary(fund, { model, apiKey: apiKeyArg, cardSummary, signal } = {}) {
  const apiKey = apiKeyArg || process.env.DASHSCOPE_API_KEY;
