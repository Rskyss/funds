...
export async function generateDetailWithRetry(fund, { retries = 2, model, cardSummary, signal } = {}) {
  let lastErr;
  for (let i = 0; i <= retries; i++) {
    try {
      return await generateFundDetailSummary(fund, { model, cardSummary, signal });
    } catch (err) {
      lastErr = err;
      if (i < retries) {
        await new Promise((r) => setTimeout(r, 1000 * (i + 1)));
