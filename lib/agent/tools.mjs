...
import { withoutStaleNav } from "../dataQuality.mjs";
...
function projectFund(fund) {
  return {
    ...
    navStaleDays: fund.navStaleDays ?? null,
  };
}

export async function filterFunds(filter) {
  // 筛选/排行不给净值停更的基金（旧数据会混进排行）；点名查询走 getFundsByCodes 仍能查到
  const all = withoutStaleNav(await loadFundsCached());
