...
import { chatCompletionWithRetry } from "../ai.mjs";
...
async function generateQuestionsByLLM({ trigger, extremeBoard, contextSnippet }) {
  ...
  const { json } = await chatCompletionWithRetry({
    model: process.env.DASHSCOPE_MODEL_STRONG || process.env.DASHSCOPE_MODEL || "qwen-plus",
    messages: [
