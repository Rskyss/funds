...
- 推荐用户去看的基金，在正文中**只用 6 位代码**提及一次（前端会自动渲染含名称、近1月/近3月/近1年/夏普/最大回撤/晨星评级/标签等的完整卡片）。**禁止**再写出基金中文名称（如禁止写"005698 华夏全球科技先锋混合(QDII)A"，只写"005698"即可）。
- 卡片下方只跟「一句话亮点」（≤30字），如"重仓 AI 算力，弹性最大但回撤也最深"。**禁止**逐项罗列卡片已有的"近3月+X%、近1年+X%、夏普 X、回撤 -X%"等数字——会和卡片重复。
- **禁止**在正文末尾复述"数据来源：xxx、xxx（详见[1][2]）"之类的引用清单，前端会自动渲染可折叠的"引用 X 条来源"区。
- 整体回答控制在 250 字以内（含风险提示）；列举多只基金时每只 ≤ 30 字。`;
...
// 千人千面：策略写在 rules/persona.md 卡片里（产品可直接改），
// 这里只把当前登录用户的画像取值翻译成「数据」塞进去，由卡片规则套用。
const PERSONA_LABELS = {
  fund_years: { none: "没买过基金", lt1: "不到 1 年", "1to3": "1-3 年", "3to5": "3-5 年", gt5: "5 年以上" },
  risk_pref: { low: "保守", mid: "平衡", high: "进取" },
  horizon: { short: "1 年内", mid: "1-3 年", long: "3 年以上" },
  amount_band: { "<10w": "10 万以内", "10-50w": "10-50 万", "50-200w": "50-200 万", ">200w": "200 万以上" },
};

export function buildPersonaDirective(profile) {
  const policy = card("persona");
  if (!policy) return "";

  const facts = [];
  const L = PERSONA_LABELS;
  if (profile?.fund_years && L.fund_years[profile.fund_years]) facts.push(`投资经验：${L.fund_years[profile.fund_years]}`);
  if (profile?.risk_pref && L.risk_pref[profile.risk_pref]) facts.push(`风险偏好：${L.risk_pref[profile.risk_pref]}`);
  if (profile?.horizon && L.horizon[profile.horizon]) facts.push(`计划持有期：${L.horizon[profile.horizon]}`);
  if (Array.isArray(profile?.regions) && profile.regions.length) facts.push(`已配置过的区域：${profile.regions.join("、")}`);
  if (profile?.amount_band && L.amount_band[profile.amount_band]) facts.push(`可投金额：${L.amount_band[profile.amount_band]}`);

  const data = facts.length
    ? facts.map((f) => `- ${f}`).join("\n")
    : "- （未填写任何偏好，按卡片「用户没填偏好时」处理）";
  return `\n\n---\n\n${policy}\n\n## 当前这位用户的画像（数据）\n${data}`;
}
