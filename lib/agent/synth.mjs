import { chatCompletion, chatCompletionStream } from "../ai.mjs";
...
export async function synthesize({ user, history = [], state, profile = null } = {}) {
  ...
  const strongModel = process.env.DASHSCOPE_MODEL_STRONG || "qwen-max";
  const fallbackModel = process.env.DASHSCOPE_MODEL_STRONG_FALLBACK || "qwen-max";
  ...
  try {
    const r = await chatCompletion({
      ...
      topP: 0.85,
      maxTokens: 900,
      ...thinkingOptions(strongModel),
    });
    return { reply: r.content.trim(), model: r.model, ok: true };
  } catch (err) {
    if (err.message?.includes("AllocationQuota.FreeTierOnly") && fallbackModel !== strongModel) {
      try {
        const r = await chatCompletion({
          model: fallbackModel,
          messages,
          ...
          topP: 0.85,
          maxTokens: 900,
          ...thinkingOptions(fallbackModel),
        });
        return { reply: r.content.trim(), model: r.model, ok: true, fallbackFrom: strongModel };
      } catch (fallbackErr) {
        return {
...
export async function synthesizeStream({ user, history = [], state, onDelta, onReasoningDelta, profile = null } = {}) {
  ...
  try {
    ...
    return { reply: r.content.trim(), reasoning: r.reasoning || "", model: r.model, ok: true };
  } catch (err) {
    if (err.message?.includes("AllocationQuota.FreeTierOnly") && fallbackModel !== strongModel) {
      try {
        const r = await chatCompletionStream({
        ...
        return { reply: r.content.trim(), reasoning: r.reasoning || "", model: r.model, ok: true, fallbackFrom: strongModel };
      } catch (fallbackErr) {
        return {
          reply: "我这边模型暂时调不通，先稍后再问一次试试。",
          model: fallbackModel,
