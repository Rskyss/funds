...
import { SYNTH_SYSTEM } from "./prompts.mjs";
...
export async function synthesize({ user, history = [], state } = {}) {
...
export async function synthesizeStream({ user, history = [], state, onDelta } = {}) {
  const toolContext = buildToolContext(state);
  const strongModel = process.env.DASHSCOPE_MODEL_STRONG || "qwen-max";
  const fallbackModel = process.env.DASHSCOPE_MODEL_STRONG_FALLBACK || "qwen-max";
  const recentTurns = (Array.isArray(history) ? history : []).slice(-6);
