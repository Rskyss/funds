...
export function fundLine(f) {
  ...
  if (f.navStaleDays) {
    // 净值停更（东财仍列着、净值不再更新，多为清盘/暂停运作）：数据是旧的，明确告诉模型别推荐
    parts.push(`⚠ 净值停更 ${f.navStaleDays} 天（净值日期 ${f.navDate || "?"}，以上收益是旧数据），可能已清盘或暂停运作，不要推荐`);
  }
...
export async function synthesize({ user, history = [], state, profile = null, creds = null } = {}) {
  const toolContext = buildToolContext(state);
  const models = creds?.model ? [creds.model] : modelChain();
...
export async function synthesizeStream({ user, history = [], state, onDelta, onReasoningDelta, profile = null, creds = null } = {}) {
  const toolContext = buildToolContext(state);
  const models = creds?.model ? [creds.model] : modelChain();
  const recentTurns = (Array.isArray(history) ? history : []).slice(-6);
  const messages = [
...
export function pickCards(state) {
  ...
  const raw = state.funds.slice(0, 12).map((f) => ({
    ...
    navStaleDays: f.navStaleDays ?? null,
