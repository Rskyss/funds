 ...
 * 返回按字段统计的回退数（restoredByField），日志按字段报比总数有意义：
 * 例如 maxDrawdown1y 在刷新后一步才重算，这里几乎每只都会“回退”，不该算进抓取失败。
 */
export function fillMissingForAll(funds, prevByCode, fields = STICKY_METRIC_FIELDS) {
  let restoredFields = 0;
  let restoredFunds = 0;
  const restoredByField = {};
  const out = funds.map((f) => {
    ...
    if (restored.length) {
      ...
      restoredFunds += 1;
      for (const k of restored) restoredByField[k] = (restoredByField[k] || 0) + 1;
    }
    return fund;
  });
  return { funds: out, restoredFields, restoredFunds, restoredByField };
}
