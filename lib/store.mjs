...
import { splitDelisted, markStaleNav } from "./dataQuality.mjs";
import { fundToRow, rowToFund, parseDate } from "./fundRow.mjs";
import { computeMaxDrawdown } from "./eastmoney.mjs";
import { isValidNavRow } from "./navValidation.mjs";
...
export async function appendNavHistory(funds) {
  const candidates = funds.map((f) => ({
    code: f.code,
    nav_date: parseDate(f.date),
    nav: f.nav,
    accum_nav: f.accumNav,
  }));
  // 注意别写成 filter(isValidNavRow)：第二个参数会收到数组下标（2026-09-01 生产事故，740 行全被拒）
  const rows = candidates.filter((row) => isValidNavRow(row));
  const dropped = candidates.length - rows.length;
  if (dropped > 0) console.warn(`[store] appendNavHistory 跳过 ${dropped} 行无效净值（为空/非正数/日期异常）`);
...
export async function saveNavHistoryRows(code, rows) {
  ...
  const data = rows
    .filter((r) => isValidNavRow({ nav: r.nav, nav_date: r.nav_date }))
    .map((r) => ({ code, nav_date: r.nav_date, nav: r.nav, accum_nav: r.accum_nav ?? null }));
  if (!data.length) return { count: 0 };
  // fetchNavHistory 一次最多 400 行，与 appendNavHistory 一样按 200 分片，避免单请求过大
  const chunkSize = 200;
...
// 最近一次读库识别出的"东财已不再返回"的基金代码（见 lib/dataQuality.mjs splitDelisted），详情接口据此 404
let delistedCodes = new Set();
export function isDelistedCode(code) {
  return delistedCodes.has(code);
}

/**
 * 读全部基金。默认隐藏东财已不再返回的基金（updated_at 比最新一批晚 3 天以上）；
 * includeDelisted: true 时返回全部（刷新时做指标回退用）。
 */
export async function getAllFunds({ includeDelisted = false } = {}) {
  const pageSize = 1000;
  let from = 0;
  const all = [];
  while (true) {
  ...
  const funds = markStaleNav(all.map(rowToFund)); // 净值停更标记（AI 投顾读库路径不经过 applyPercentileScores，这里保证字段存在）
  const { active, delisted } = splitDelisted(funds);
  delistedCodes = new Set(delisted.map((f) => f.code));
  if (delisted.length && !getAllFunds.loggedDelisted) {
    getAllFunds.loggedDelisted = true;
    console.log(`[store] 隐藏东财已不再返回的基金 ${delisted.length} 只：${delisted.map((f) => `${f.code} ${f.name}`).join("、")}`);
  }
  return includeDelisted ? funds : active;
}
...
/** 全部基金的基金公司字段（回填脚本核对官网对照用） */
export async function listFundCompanies() {
  const { data, error } = await supabaseAdmin.from("fund_details").select("code,company_id,company_name").limit(5000);
  if (error) throw new Error(`listFundCompanies 失败: ${error.message}`);
  return data || [];
}
...
export async function saveFundDetail(detail) {
  const row = {
    ...
    company_name: detail.companyName || null,
...
// ===== 匿名行为事件（events 表）=====
// 仅记录匿名行为：anon_id（随机匿名编号）/ user_id（登录时才有）/ type / code / payload
export async function insertEvents(rows) {
  if (!Array.isArray(rows) || !rows.length) return;
  const { error } = await supabaseAdmin.from("events").insert(rows);
  if (error) throw new Error(`insertEvents 失败: ${error.message}`);
}

export async function getEventsSince(sinceIso, { limit = 20000 } = {}) {
  const { data, error } = await supabaseAdmin
    .from("events")
    .select("anon_id, user_id, type, code, payload, created_at")
    .gte("created_at", sinceIso)
    .order("created_at", { ascending: false })
    .limit(limit);
  if (error) throw new Error(`getEventsSince 失败: ${error.message}`);
  return data || [];
}
